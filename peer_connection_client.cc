...
rtc::AsyncSocket* CreateClientSocket(int family) {
  ...
  RTC_LOG(WARNING)
        << "ppt, CreateClientSocket: thread: " << !thread;
...
void PeerConnectionClient::Connect2janusServer(bool isp2p) {//1: is p2p; 0: not p2p
    //std::shared_ptr<PeerFactory> _peerFactory;
    std::cout<<"PeerConnectionClient::Connect2janusServer\n";
...
void PeerConnectionClient::OnRead(rtc::AsyncSocket* socket) {
  ...
  if (ReadIntoBuffer(socket, &control_data_, &content_length)) {
	...
	RTC_LOG(LS_INFO) << "ppt, in PeerConnectionClient::OnRead, control_data_: " << control_data_;
    bool ok =
      ...
      if (my_id_ == -1) {
        ...
        if (content_length) {
          ...
          while (pos < control_data_.size()) {
            ...
            std::string name;
            bool connected;
			RTC_LOG(LS_ERROR) << "ppt, in PeerConnectionClient::OnRead, control_data_: " << control_data_;
            if (ParseEntry(control_data_.substr(pos, eol - pos), &name, &id,
                           &connected) &&
                id != my_id_) {
              peers_[id] = name;
			  RTC_LOG(LS_INFO) << "ppt, in PeerConnectionClient::OnRead, connected!!"<< id << ", " << name;;
			  std::cout << "ppt, in PeerConnectionClient::OnRead, connected!!"<< id << ", " << name << std::endl;;
...
void PeerConnectionClient::OnHangingGetRead(rtc::AsyncSocket* socket) {
  ...
  if (ReadIntoBuffer(socket, &notification_data_, &content_length)) {
    size_t peer_id = 0, eoh = 0;
	RTC_LOG(LS_INFO) << "ppt, in PeerConnectionClient::OnHangingGetRead, notification_data_: " 
		<< notification_data_;
    bool ok =
    ...
    if (ok) {
      ...
      if (my_id_ == static_cast<int>(peer_id)) {
        ...
        if (ParseEntry(notification_data_.substr(pos), &name, &id,
          ...
          if (connected) {
            peers_[id] = name;
			RTC_LOG(LS_INFO) << "ppt, in PeerConnectionClient::OnHangingGetRead, connected!!, id, name: " 
				<< id << ", " << name;
            callback_->OnPeerConnected(id, name);
          } else {
            peers_.erase(id);
